...
class WebRTCClient(
    ...
    private val audioProcessingFactory: AudioProcessingFactory
) {
    ...
    fun startLocalVideoCapture(localVideoOutput: SurfaceViewRenderer) {
        try {
            ...
            if (surfaceTextureHelper == null) {
                println("WebRTCClient:startLocalVideoCapture: surfaceTextureHelper is null")
                return
            }

            if (videoCapturer == null) {
                println("WebRTCClient:startLocalVideoCapture videoCapturer is null")
                return
            }
            ...
            println("WebRTCClient:Error starting local video capture")
        }
    }
            ...
            override fun onCreateSuccess(desc: SessionDescription?) {
                setLocalDescription(object : SdpObserver {
                    override fun onSetFailure(p0: String?) {
                        println("WebRTCClient:onCreateSuccess: onSetFailure")
                    }
    ...
    private fun PeerConnection.answer(sdpObserver: SdpObserver) {
        createAnswer(object : SdpObserver by sdpObserver {
            override fun onCreateSuccess(p0: SessionDescription?) {
                setLocalDescription(object : SdpObserver {
                    override fun onSetFailure(p0: String?) {
                        println("WebRTCClient:onCreateSuccess:answer onSetFailure")
                    }
    ...
    fun onRemoteSessionReceived(sessionDescription: SessionDescription) {
        peerConnection?.setRemoteDescription(object : SdpObserver {
            override fun onSetFailure(p0: String?) {
                println("WebRTCClient:onRemoteSessionReceived: onSetFailure")
            }
